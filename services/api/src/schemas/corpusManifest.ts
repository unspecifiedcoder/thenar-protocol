 ...
 * which omits the server-computed `corpus_root`/`episode_count`).
 */
...
/** Body accepted by `POST /v1/corpora`: `corpus_root`/`episode_count` are server-computed. */
export const CorpusManifestInput = CorpusManifestSchema.innerType()
  .omit({ corpus_root: true, episode_count: true })
  .strict();
